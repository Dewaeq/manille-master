...
impl App {
    ...
    pub async fn run(&mut self) {
        loop {
            ...
            if self.game.round.phase() == RoundPhase::PickTrump {
    ...
    fn render_bot_icons(&self) {
        ...
        for i in 1..4 {
            let pos = self.get_bot_position(i);
    ...
    fn render_cards(&mut self) {
        ...
        for moving_card in &mut self.moving_cards {
            moving_card.draw();
            moving_card.update();
        }
    }
    ...
    fn get_bot_position(&self, bot: usize) -> Vec2 {
        let (width, height) = screen_size();
        let bot_positions = [
            ...
            if let Some(action) = self.game.load_ai_move() {
                ...
                match action {
                    Action::PlayCard(card) => {
                        let ui_card = UiCard::new(card, self.get_bot_position(turn), false);
                        self.play_card(ui_card);
                    }
